...
public class Svc2021Loader : DataSetLoader
{
    ...
    public struct SignatureFile
    {
        ...
        {
            this.File = file;
            var pathParts = file.Contains("/") ? file.Split('/') : file.Split('\\');
    ...
    /// </summary>
    public string DataBaseZipBase64 { get; set; }
    ...
    public Svc2021Loader(string dataBaseZipBase64, bool standardFeatures, Predicate<Signer> signerFilter = null)
    {
    ...
    public override IEnumerable<Signer> EnumerateSigners(Predicate<Signer> signerFilter)
    {
        ...
        signatureGroups = zip.Entries.Where(f => f.FullName.StartsWith("DeepSignDB") && f.Name.EndsWith(".txt"))
